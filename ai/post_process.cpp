...
static char *labels[OBJ_CLASS_NUM] = {0};
...
int loadLabelName(const char *locationFilename)
{
    ...
    readLines(locationFilename, labels, OBJ_CLASS_NUM);
    return 0;
}
...
static int nms(int validCount, std::vector<float> &outputLocations, std::vector<int> classIds, std::vector<int> &order, int filterId, float threshold)
{
    for (int i = 0; i < validCount; ++i)
    {
        if (order[i] == -1)
        {
            // printf("i:%d compared before\n", i);
            continue;
        }

        if (classIds[i] != filterId)
        {
            // printf("i:%d classIds[i]:%d != filterId:%d\n", i, classIds[i], filterId);
            continue;
        }
        int n = order[i];
        for (int j = i + 1; j < validCount; ++j)
        {
            ...
            // printf("i:[%d] j[%d]: n %d -> m %d, filterId:%d \n", i, j, n, m, filterId);
...
{
    int ret = 0;
    ret = loadLabelName(LABEL_NALE_TXT_PATH);

    for (int i = 0; i < OBJ_CLASS_NUM; i++)
    {
        printf("init :::::::::: label %d %s\n", i, labels[i]);
    }
    if (ret < 0)
    {
        return;
                 ...
                 std::vector<int32_t> &qnt_zps, std::vector<float> &qnt_scales)
{
    ...
    memset(&ai_result, 0, sizeof(nn_object_array_t));
    ...
    // 对结果做排序, 置信度大到小, 同时记录index, 类似map排序
    quick_sort_indice_inverse(objProbs, 0, validCount - 1, indexArray);

    for (int i = 0; i < validCount; i++)
    {
        // printf("objProbs[%d] = %f indexArray[%d] = %d \n", i, objProbs[i], i, indexArray[i]);
    }
    ...
    for (int i = 0; i < validCount; ++i)
    {
        ...
        ai_result.object[last_count].box.left = (int)(clamp(x1, 0, model_in_w) / scale_w);
        ai_result.object[last_count].box.top = (int)(clamp(y1, 0, model_in_h) / scale_h);
        ai_result.object[last_count].box.right = (int)(clamp(x2, 0, model_in_w) / scale_w);
        ai_result.object[last_count].box.bottom = (int)(clamp(y2, 0, model_in_h) / scale_h);
        ai_result.object[last_count].score = obj_conf;
        ai_result.object[last_count].cls_idx = id;
        char *label = labels[id];
        printf("label: %s\n", labels[id]);
        strncpy(ai_result.object[last_count].name, label, OBJ_NAME_MAX_SIZE);

        // strncpy(group->object[last_count].name, labels[id], OBJ_NAME_MAX_SIZE);
        // sprintf(group->object[last_count].name, "%s", labels[id]);
        // printf("group->object[last_count].name %s\n", group->object[last_count].name);

        // memcpy(group->object[last_count].name, labels[id], 64);

        printf("result %2d: %s (%4d, %4d, %4d, %4d)\n", id, ai_result.object[last_count].name, ai_result.object[last_count].box.left, ai_result.object[last_count].box.top,
               ai_result.object[last_count].box.right, ai_result.object[last_count].box.bottom);

        // printf("result %2d: (%4d, %4d, %4d, %4d), %s\n", i, group->results[last_count].box.left, group->results[last_count].box.top,
        //        group->results[last_count].box.right, group->results[last_count].box.bottom, label);
        last_count++;
    }
    ai_result.count = last_count;
